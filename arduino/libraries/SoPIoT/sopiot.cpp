...
char *dtostrf_cap(double val, signed char width, unsigned char prec,
                  char *sout) {
...
void Value::set_name(const char *name) { name_ = strdup(name); }
...
bool Value::capVal2str(char *buffer) {
  ...
  switch (value_classifier_) {
    ...
    case BOOL: {
      ...
      /*
              if (*(int*)val == 1) {
                      len = sprintf(buffer, "{\"type\" : \"bool\" , \"value\" :
         1}");
              }
              else {
                      len = sprintf(buffer, "{\"type\" : \"bool\" , \"value\" :
...
void Attribute::set_name(const char *name) { name_ = strdup(name); }
...
void Argument::GetInformation(char *buffer) {
  switch (arg_type_) {
    case BOOL:
      ...
      break;
    case INTEGER:
      snprintf(buffer, MAX_BUFFER_SIZE, "%s\tint\t%d\t%d\t", name_,
               *(int *)min_, *(int *)max_);
      break;
    case STRING:
      snprintf(buffer, MAX_BUFFER_SIZE, "%s\tstring\t%d\t%d\t", name_,
...
void Function::set_name(const char *name) { name_ = strdup(name); }
...
void Thing::set_class_name(char *class_name) { class_name_ = class_name; }
...
void Thing::sendAliveMessageRightNow() {
  char *pszDummy = (char *)"dummy";
...
void Thing::checkSerialForValue() {
...
void Thing::set_connect_handler(void (*connectHandler)()) {
  connect_handler_ = connectHandler;
}

void Thing::set_disconnect_handler(void (*disconnectHandler)()) {
...
uint16_t Thing::bswap(const uint16_t val) { return (val << 8) | (val >> 8); }
...
void Thing::broadcast() {
  ...
  zbee_tx_ = ZBTxRequest(addr64, addr16, 10, ZB_TX_UNICAST, message_buffer_,
    ...
    if (topic_id == functions_[i]->id_1003()) {
      ...
      if (t_name[0] == ' ')  // No scenario name provided
      {
        snprintf(buffer, MAX_BUFFER_SIZE,
                 "{\"function\" : \"%s\" , \"error\" : %d }",
                 functions_[i]->name(), success);
        publish(QOS_FLAG, functions_[i]->id_2004_deprecated(), buffer,
                strlen(buffer));
