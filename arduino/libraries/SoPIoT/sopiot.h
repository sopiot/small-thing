#ifndef _SMALL_THING_SOPIOT_H_
#define _SMALL_THING_SOPIOT_H_

#include <Arduino.h>
#include <Stream.h>
#include <XBee.h>

#include "sopiot_defines.h"
 ...
 */
/**
 * @details A programmer should declare this class and set id, set serial, add
 * values and functions. then call .Setting on setting tab, and Publish in
 * loop(). it acts like as a bridge between thing and middleware. it is very
 * similar to the Class Client in thing_client. please refer that documentation.
 */
...
*/
 ...
 private:
   ...
   * @param connectHandler the void function that programmer wants to activate
   * when arduino connects to the gateway.
   */
  void set_connect_handler(void (*connectHandler)());

  /**
   * set function that acts when arduino disconnects from the gateway
   * @param disconnectHandler the void function that programmer wants to
   * activate when arduino disconnects from the gateway.
   */
...
#endif
  /*
  void willtopic(const uint8_t flags, const char* will_topic, const bool update
  = false); void willmsg(const void* will_msg, const uint8_t will_msg_len, const
  bool update = false);
*/
