...
// This value is determined by the largest packet size (100 byte payload +
// 64-bit address + option byte and rssi byte) of a series 1 radio
...
class XBeeResponse {
 public:
  ...
  /**
   * Returns the length of the frame data: all bytes after the api id, and prior
   * to the checksum Note up to release 0.1.2, this was incorrectly including
   * the checksum in the length.
   */
  uint8_t getFrameDataLength();
  void setFrameData(uint8_t* frameDataPtr);
  /**
   * Returns the buffer that contains the response.
   * Starts with byte that follows API ID and includes all bytes prior to the
   * checksum Length is specified by getFrameDataLength() Note: Unlike Digi's
   * definition of the frame data, this does not start with the API ID.. The
   * reason for this is all responses include an API ID, whereas my frame data
...
#ifdef SERIES_2
  /**
   * Call with instance of ZBTxStatusResponse class only if getApiId() ==
   * ZB_TX_STATUS_RESPONSE to populate response
   */
  void getZBTxStatusResponse(XBeeResponse& response);
  /**
   * Call with instance of ZBRxResponse class only if getApiId() ==
   * ZB_RX_RESPONSE to populate response
   */
  void getZBRxResponse(XBeeResponse& response);
  /**
   * Call with instance of ZBRxIoSampleResponse class only if getApiId() ==
   * ZB_IO_SAMPLE_RESPONSE to populate response
   */
  void getZBRxIoSampleResponse(XBeeResponse& response);
#endif
#ifdef SERIES_1
  /**
   * Call with instance of TxStatusResponse only if getApiId() ==
   * TX_STATUS_RESPONSE
   */
  ...
  /**
   * Call with instance of Rx16IoSampleResponse only if getApiId() ==
   * RX_16_IO_RESPONSE
   */
  void getRx16IoSampleResponse(XBeeResponse& response);
  /**
   * Call with instance of Rx64IoSampleResponse only if getApiId() ==
   * RX_64_IO_RESPONSE
   */
  void getRx64IoSampleResponse(XBeeResponse& response);
#endif
  /**
   * Call with instance of AtCommandResponse only if getApiId() ==
   * AT_COMMAND_RESPONSE
   */
  void getAtCommandResponse(XBeeResponse& responses);
  /**
   * Call with instance of RemoteAtCommandResponse only if getApiId() ==
   * REMOTE_AT_COMMAND_RESPONSE
   */
  void getRemoteAtCommandResponse(XBeeResponse& response);
  /**
   * Call with instance of ModemStatusResponse only if getApiId() ==
   * MODEM_STATUS_RESPONSE
   */
  void getModemStatusResponse(XBeeResponse& response);
  /**
   * Returns true if the response has been successfully parsed and is complete
   * and ready for use
   */
  bool isAvailable();
   ...
   * Error codes include: CHECKSUM_FAILURE, PACKET_EXCEEDS_BYTE_ARRAY_LENGTH,
...
class RxDataResponse : public XBeeResponse {
 public:
  RxDataResponse();
  /**
   * Returns the specified index of the payload.  The index may be 0 to
   * getDataLength() - 1 This method is deprecated; use uint8_t* getData()
   */
  uint8_t getData(int index);
  /**
   * Returns the payload array.  This may be accessed from index 0 to
   * getDataLength() - 1
   */
  uint8_t* getData();
  /**
...
// we maintain a pointer to each type of response, when a response is parsed, it
// is allocated only if NULL can we allocate an object in a function?
...
class RxResponse : public RxDataResponse {
 public:
  RxResponse();
  // remember rssi is negative but this is unsigned byte so it's up to you to
  // convert
  uint8_t getRssi();
  uint8_t getOption();
 ...
 * Users should never create an instance of this class; instead use an subclass
 * of this class It is recommended to reuse Subclasses of the class to conserve
 * memory <p/> This class allocates a buffer to
 */
class XBeeRequest {
 public:
  /**
  ...
  /**
   * Sets the frame id.  Must be between 1 and 255 inclusive to get a TX status
   * response.
   */
   ...
   */
  uint8_t getApiId();
  // setting = 0 makes this a pure virtual function, meaning the subclass must
  // implement, like abstract in java
  /**
   * Starting after the frame id (pos = 0) and up to but not including the
   * checksum Note: Unlike Digi's definition of the frame data, this does not
   * start with the API ID. The reason for this is the API ID and Frame ID are
   * common to all requests, whereas my definition of frame data is only the API
   * specific data.
   */
 ...
 * This class provides methods for sending and receiving packets with an XBee
 * radio via the serial port. The XBee radio must be configured in API (packet)
 * mode (AP=2) in order to use this software. <p/> Since this code is designed
 * to run on a microcontroller, with only one thread, you are responsible for
 * reading the data off the serial buffer in a timely manner.  This involves a
 * call to a variant of readPacket(...). If your serial port is receiving data
 * faster than you are reading, you can expect to lose packets. Arduino only has
 * a 128 byte serial buffer so it can easily overflow if two or more packets
 * arrive without a call to readPacket(...) <p/> In order to conserve resources,
 * this class only supports storing one response packet in memory at a time.
 * This means that you must fully consume the packet prior to calling
 * readPacket(...), because calling readPacket(...) overwrites the previous
 * response. <p/> This class creates an array of size MAX_FRAME_DATA_SIZE for
 * storing the response packet.  You may want to adjust this value to conserve
 * memory.
 *
 * \author Andrew Rapp
 */
class XBee {
 public:
  XBee();
  /**
   * Reads all available serial bytes until a packet is parsed, an error occurs,
   * or the buffer is empty. You may call
   * <i>xbee</i>.getResponse().isAvailable() after calling this method to
   * determine if a packet is ready, or <i>xbee</i>.getResponse().isError() to
   * determine if a error occurred. <p/> This method should always return
   * quickly since it does not wait for serial data to arrive. You will want to
   * use this method if you are doing other timely stuff in your loop, where a
   * delay would cause problems. NOTE: calling this method resets the current
   * response, so make sure you first consume the current response
   */
  void readPacket();
  /**
   * Waits a maximum of <i>timeout</i> milliseconds for a response packet before
   * timing out; returns true if packet is read. Returns false if timeout or
   * error occurs.
   */
  bool readPacket(int timeout);
  /**
   * Reads until a packet is received or an error occurs.
   * Caution: use this carefully since if you don't get a response, your Arduino
   * code will hang on this call forever!! often it's better to use a timeout:
   * readPacket(int)
   */
  ...
  /**
   * Specify the serial port.  Only relevant for Arduinos that support multiple
   * serial ports (e.g. Mega)
   */
 ...
 private:
  ...
  bool _escape;
  // current packet position for response.  just a state variable for packet
  // parsing and has no relevance for the response otherwise
  uint8_t _pos;
  // last byte read
  uint8_t b;
...
class PayloadRequest : public XBeeRequest {
 public:
  PayloadRequest(uint8_t apiId, uint8_t frameId, uint8_t* payload,
  ...
  /**
   * Sets the length of the payload to include in the request.  For example if
   * the payload array is 50 bytes and you only want the first 10 to be included
   * in the packet, set the length to 10. Length must be <= to the array length.
   */
 ...
 * radio. This class does not perform any validation of packet size and there
 * will be no indication if the packet is too large, other than you will not get
 * a TX Status response. The datasheet says 100 bytes is the maximum, although
 * that could change in future firmware.
 */
class Tx16Request : public PayloadRequest {
 public:
  Tx16Request(uint16_t addr16, uint8_t option, uint8_t* payload,
...
class Tx64Request : public PayloadRequest {
 public:
  Tx64Request(XBeeAddress64& addr64, uint8_t option, uint8_t* payload,
              uint8_t payloadLength, uint8_t frameId);
  /**
 ...
 * radio. This class does not perform any validation of packet size and there
 * will be no indication if the packet is too large, other than you will not get
 * a TX Status response. The datasheet says 72 bytes is the maximum for ZNet
 * firmware and ZB Pro firmware provides the ATNP command to get the max
 * supported payload size.  This command is useful since the maximum payload
 * size varies according to certain settings, such as encryption. ZB Pro
 * firmware provides a PAYLOAD_TOO_LARGE that is returned if payload size
 * exceeds the maximum.
 */
...
class AtCommandRequest : public XBeeRequest {
 public:
  ...
  /**
   * Clears the optional commandValue and commandValueLength so that a query may
   * be sent
   */
