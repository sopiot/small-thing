...
Value::Value(int (*value)(void)) {
	Initialize();
	set_value(value);
}


Value::Value(double (*value)(void)) {
	Initialize();
	set_value(value);
}

Value::Value(bool (*value)(void)) {
	Initialize();
	set_value(value);
}

Value::Value(int (*value)(void), int min ,int max) {
	Initialize();
	set_value(value);
	set_min(min);
	set_max(max);
...
Value::Value(double (*value)(void), double min, double max) {
	Initialize();
	set_value(value);
	set_min(min);
	set_max(max);
...
bool Value::value_changed(void* cur) {
	...
	switch(value_classifier_) {
		case STRING:
			if (strncmp((char*)cur, (char*)prev_, *(int*)max_) != 0) changed = true;
...
bool Argument::Check_valid_and_set(char* val)
{
	...
	switch (arg_type_)
	{
		...
		{
			int nval = atoi(val);
			if (nval < *(int*)min_ || nval > *(int*)max_) flag = false;
			else *(int*)value_ = nval;
			break;
		}
		case DOUBLE:
		{
			double dval = atof(val);
			if (dval < *(double*)min_ || dval > *(double*)max_) flag = false;
			else *(double*)value_ = dval;
			break;
		}
		case STRING:
		{
			int len = strlen(val);
			if (len < *(int*)min_ || len > *(int*)max_) flag = false;
			else strncpy((char*)value_, val, len+1);
			break;
		}
		default:
			break;
...
Function::Function(const char * name, VoidFunction func, int nArguments) {
	...
	if(nArguments > 0) ptsArguments_ = (Argument**)malloc(sizeof(Argument*)*nArguments);
}
...
//	int index = 0;
	...
	switch(function_classifier_)
	{
      ...
      //Dowhan Added : does FunctionAttribute should be here?
			break;
		default: // error!
			CPDBG(F("ERROR!"));
			break;
...
ThingClient::ThingClient(const char* class_name, int alive_cycle, Stream& serial) :
waiting_for_response_(true),
response_wait_for_(ADVERTISE),
connected_(false),
message_id_(0),
...
void ThingClient::checkSerial()
{
	...
	while(zbee_.readPacket(timeout)) {
		if(zbee_.getResponse().getApiId() == ZB_TX_STATUS_RESPONSE){
			...
			if(txStatus.isSuccess()) CPDBG(F("Send Success... waiting for response"));
			else CPDBG(F("Send Failed"));
		}
		...
		else if(zbee_.getResponse().isError()) CPDBG(F("Error"));
		else CPDBG(F("Whats up?"));	
	}
}
...
void ThingClient::dispatch() {
	...
	case ADVERTISE:
		if (!waiting_for_response_ || (response_wait_for_ != ADVERTISE && response_wait_for_ != GWINFO)) return;
		CPDBG(F("ADVERTISE"));
	...
	case REGACK:
		if (!waiting_for_response_ || response_wait_for_ != REGACK) return;
...
void ThingClient::publishHandler(const msg_publish* msg) {
	...
	if (topic_id == id_1001_)
	{
        ...
        CPDBG(F("Error no :"));
        CPDBG(buffer);
		if (errNo == '0') registered_ = !registered_;
...
void ThingClient::pingreqHandler(){
	if(zbee_rx_.getRemoteAddress16() == gateway_address_16_) pingresp(0);
	else pingresp(1);
}
 ...
 * not_used functions
 ...
 void ThingClient::willtopicreqHandler(const message_header* msg) {
 }

 void ThingClient::willmsgreqHandler(const message_header* msg) {
 }


void ThingClient::willtopicrespHandler(const msg_willtopicresp* msg) {
}

void ThingClient::willmsgrespHandler(const msg_willmsgresp* msg) {
}

void ThingClient::willtopic(const uint8_t flags, const char* will_topic, const bool update) {
	if (will_topic == NULL) {
		message_header* msg = reinterpret_cast<message_header*>(message_buffer_);
		...
		msg_willtopic* msg = reinterpret_cast<msg_willtopic*>(message_buffer_);
	...
	response_wait_for_ = WILLTOPICREQ;
}

void ThingClient::willmsg(const void* will_msg, const uint8_t will_msg_len, const bool update) {
	msg_willmsg* msg = reinterpret_cast<msg_willmsg*>(message_buffer_);
...
void ThingClient::regack(const uint16_t topicId, const uint16_t messageId, const return_code_t return_code) {
	msg_regack* msg = reinterpret_cast<msg_regack*>(message_buffer_);
...
void ThingClient::puback(const uint16_t topicId, const uint16_t messageId, const return_code_t return_code) {
	msg_puback* msg = reinterpret_cast<msg_puback*>(message_buffer_);
...
void ThingClient::subscribeByName(const uint8_t flags, const char* topic_name) {
	++message_id_;
...
void ThingClient::unsubscribeByName(const uint8_t flags, const char* topic_name) {
	++message_id_;
